...
public class PicTest {
    ...
    public static void main(String args[]) throws Exception{
        String tableName = "wordcountZ";
        Configuration conf =  new Configuration();
        conf.set(TableOutputFormat.OUTPUT_TABLE, tableName);
        createHBaseTable(tableName);
        //Job job = new Job(conf, "WordCountHbase");
        String input = args[0]; //2
        Job job = new Job(conf, "WordCountHbase table with "+ input); //2
        job.setJarByClass(PicTest.class);
        ...
        FileInputFormat.addInputPath(job, new Path(input)); //2
       //FileInputFormat.addInputPath(job, new Path(args[0]));  
        //本地读入Input path does not exist: input or file:/home/hadoop/workspace/WordCountHbaseFu/input
        //hdfs： hdfs://localhost:9000/user/hadoop/input
